...
describe('signInWithAppleService', () => {
  it('should return AccessToken and User from API response', async () => {
    const result = await signInWithAppleService(mockAuthService, mockHttpClient);
    expect(result).toEqual(mockAuthResponse);
  });

  it('should call signInWithApple on authService', async () => {
    const spy = jest.fn().mockResolvedValue({ token: 'mock-token', provider: 'apple' });
    await signInWithAppleService({ ...mockAuthService, signInWithApple: spy }, mockHttpClient);
    expect(spy).toHaveBeenCalledTimes(1);
  });

  it('should post idToken to auth/apple endpoint', async () => {
    const postSpy = jest.fn().mockResolvedValue({ data: mockAuthResponse, status: 200 });
    await signInWithAppleService(mockAuthService, { ...mockHttpClient, post: postSpy });
    expect(postSpy).toHaveBeenCalledWith('auth/apple', { idToken: 'mock-token' });
  });
});
